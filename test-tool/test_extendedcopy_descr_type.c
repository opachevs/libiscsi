...
int init_xcopybuf(unsigned char *buf, int tgt_desc_type, int seg_desc_type,
	...
	offset += populate_tgt_desc(buf + offset, tgt_desc_type, LU_ID_TYPE_LUN, 0,
			0, 0, 0, sd);
	offset += populate_tgt_desc(buf + offset, tgt_desc_type, LU_ID_TYPE_LUN, 0,
			0, 0, 0, sd);
	*tgt_desc_len = offset - XCOPY_DESC_OFFSET;
	if (seg_desc_type == BLK_TO_BLK_SEG_DESCR)
		...
		offset += populate_seg_desc_hdr(buf + offset, seg_desc_type, 0, 0, 0,
				0);
...
void test_extendedcopy_descr_type(void) {
	...
	logging(LOG_VERBOSE, "Test EXTENDED COPY unsupported descriptor types");

	CHECK_FOR_DATALOSS;

	alloc_len = XCOPY_DESC_OFFSET + 2 * get_desc_len(IDENT_DESCR_TGT_DESCR)
			+ get_desc_len(BLK_TO_BLK_SEG_DESCR);
	data.data = alloca(alloc_len);
	xcopybuf = data.data;
	memset(xcopybuf, 0, alloc_len);

	logging(LOG_VERBOSE, "Send Fibre Channel N_Port_Name target descriptor");
	...
	data.size = init_xcopybuf(xcopybuf, IDENT_DESCR_TGT_DESCR,
			STRM_TO_STRM_SEG_DESCR, &tgt_desc_len, &seg_desc_len);
	populate_param_header(xcopybuf, 0, 0, LIST_ID_USAGE_DISABLE, 1,
				tgt_desc_len, seg_desc_len, 0);
