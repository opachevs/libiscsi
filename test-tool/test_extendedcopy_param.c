...
void test_extendedcopy_param(void) {
	...
	data.size = XCOPY_DESC_OFFSET + 2 * get_desc_len(IDENT_DESCR_TGT_DESCR) - 1;
	EXTENDEDCOPY(sd, &data, EXPECT_PARAM_LIST_LEN_ERR);

	logging(LOG_VERBOSE,
			"Test parameter list length truncating segment descriptor");
	data.size = XCOPY_DESC_OFFSET + 2 * get_desc_len(IDENT_DESCR_TGT_DESCR) + get_desc_len(BLK_TO_BLK_SEG_DESCR) - 1;
	EXTENDEDCOPY(sd, &data, EXPECT_PARAM_LIST_LEN_ERR);
